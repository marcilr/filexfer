...
%% ============ Scheduling Usage Collection Jobs =================
%% ============ Scheduling Usage Collection Jobs =================
\subsection{Scheduling Usage Collection Jobs}
Jobs are scheduled using a web interface at
\texttt{nms.operations.gci.com/relevance}. Navigate to the
``\texttt{FileXfer}'' application and click the
``\texttt{File Transfer Jobs}'' link.  Job execution happens on
\texttt{prod-prov4-cdr1.operations.gci.com}.  A cron job executes
every minute from \texttt{/etc/cron.d/filexfer} to kick off the
various \texttt{filexfer} scripts.\footnote{\href{http://oss-wiki.operations.gci.com/dev/index.php/Scheduling\_Usage\_Collection\_Jobs}{Scheduling Usage Collection Jobs}}
...
possible for filexfer to miss certain scheduling intervals.\\
\\
...
0, 15, 30, or 45 in the minute field of the job schedule. These
